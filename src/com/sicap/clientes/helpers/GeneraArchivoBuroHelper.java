...
import com.sicap.clientes.dao.BitacoraBuroCirculoDAO;
import com.sicap.clientes.dao.BitacoraBuroCirculoTotalesDAO;
...
public class GeneraArchivoBuroHelper {

	public static void generaArchivo ()throws Exception {

		try{
	    	Calendar cal  = Calendar.getInstance();
			String periodoReporte = FormatUtil.deleteChar(Convertidor.dateToStringBuro(cal.getTime()),'/');
			BufferedWriter out = new BufferedWriter(new FileWriter(ClientesConstants.RUTA_BASE_ARCHIVOS+"BuroCirculo\\FINAFIRM-"+periodoReporte+".txt"));
                        System.out.println("out "+ClientesConstants.RUTA_BASE_ARCHIVOS+"BuroCirculo\\FINAFIRM-"+periodoReporte+".txt");
...
//			Long saldoTotalActual = new Long(0);
			...
			for ( int paso=1; paso<5; paso++){
				...
				for( int j=0; j<clientes.length; j++ ){
					//Si el cliente no est� cancelado
					if ( clientes[j].situacionActualCredito != 4 ){
						...
						//Verifica si el cliente est� en la tabla de clientes morosos
						...
						//Calcula el MOP dependiendo del n�mero de d�as de mora
						if ( diasMora>0 ){
			...
			System.out.println("Excepcion en Main CargaBuro : "+ e.getMessage());
			e.printStackTrace();
		}
	}
}
