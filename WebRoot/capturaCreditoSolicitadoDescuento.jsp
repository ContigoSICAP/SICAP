...
TreeMap catPlanDescuento  = CatalogoHelper.getCatalogoPlanesSellFinance();
...
if ( cliente.solicitudes!=null && idSolicitud>0 ){
	...
	catEstatus = CatalogoHelper.getCatalogoEstatusDN(solicitud.estatus);
	...
	function guardaCreditoSolicitadoConsumo(){
		...
		if( <%=solicitud.tipoOperacion%>!=<%=ClientesConstants.SELL_FINANCE%>){
			...
			if ( window.document.forma.marca.value==0 ){
				alert('Debe seleccionar el convenio');
				return false;
			}
			if ( window.document.forma.producto.value==0 ){
				alert('Debe seleccionar el segmento');
				return false;
			}
		}
...
<table border="0" cellpadding="0" width="100%">
	...
	<td width="50%" align="right">Plan</td>
  	...
  	<select name="marca" size="1">
			<%=HTMLHelper.displayComboProductosSellFinance(catPlanDescuento, sellFinance.idMarca)%>
	</select>
  	</td>
	</tr>
	<tr>
	...
	<input type=hidden size="7" name="numeroFactura" value="<%=cliente.solicitudes[indiceSolicitud].numCheque%>" readonly="readonly" class="soloLectura" maxlength="7">
	</tr>
	<tr>
