...
<script>
	...
	function guardaDatosCliente(tipoOper){
		window.document.forma.command.value='guardaDatosClienteVivienda';
		...
		if ( window.document.forma.entidadNacimiento.value==0 ){
			alert('Debe capturar una entidad de nacimiento');
			return false;
		}
...
<%
	...
	if ( clienteAyuda != null ){
		...
		cliente.entidadNacimiento = clienteAyuda.entidadNacimiento;
	}
